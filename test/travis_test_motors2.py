...
class MotorTest(unittest.TestCase):
	...
	def test_put_freq(self):
		...
		self.file_check("rtmotr_raw_l0", m.left_hz, "wrong left value from motor_raw")
		self.file_check("rtmotr_raw_r0", m.right_hz, "wrong left value from motor_raw")
	...
	def test_on_off(self):
		...
		ret = off()
		self.assertEqual(ret.success, True, "motor off does not successed")
		self.assertEaual(ret.message, "OFF", "motor off wrong message")
		with open("/dev/rtmotoren0", "r") as f:
			data = f.readline()
			self.assertEqual(data, "1\n", "wrong value in rtmotor0 at motor on")

if __name__=='__main__':
	rospy.init_node('traveis_test_motors')
